    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        if (file.mimetype.startsWith("image/")) {
          image = `/uploads/crime/${file.filename}`;
          mediaType = "image";
        } else if (file.mimetype.startsWith("video/")) {
          video = `/uploads/crime/${file.filename}`;
          mediaType = "video";
        }
      });
    }
...
exports.getEditNews = async (req, res) => {
  try {
    const news = await crime.findById(req.params.id);
    if (!news) {
      req.flash("error", "News not found");
      return res.redirect("/admin/crime");
    }
...
exports.postUpdateNews = async (req, res) => {
  try {
    ...
    await crime.findByIdAndUpdate(req.params.id, {
      headline,
      location,
      description,
      highlightText: highlights,
    ...
    res.redirect(`/admin/crime/edit/${req.params.id}`);
  }
};
...
exports.downloadVideoWithOverlay = async (req, res) => {
  try {
    ...
    // file paths
    const videoPath = path.join(__dirname, "../../../uploads/crime", path.basename(news.video));
