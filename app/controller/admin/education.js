...
exports.postCreateNews = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        if (file.mimetype.startsWith("image/")) {
          image = `/uploads/education/${file.filename}`;
          mediaType = "image";
        } else if (file.mimetype.startsWith("video/")) {
          video = `/uploads/education/${file.filename}`;
          mediaType = "video";
        }
      });
    }
    ...
    res.redirect("/admin/education/create");
  }
};
...
exports.getEditNews = async (req, res) => {
  try {
    const news = await education.findById(req.params.id);
    if (!news) {
...
exports.postUpdateNews = async (req, res) => {
  try {
    ...
    res.redirect(`/admin/education/edit/${req.params.id}`);
  }
};
...
exports.downloadVideoWithOverlay = async (req, res) => {
  try {
    ...
    // file paths
    const videoPath = path.join(__dirname, "../../../uploads/education", path.basename(news.video));
