...
exports.postCreateNews = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        if (file.mimetype.startsWith("image/")) {
          image = `/uploads/entertainment/${file.filename}`;
          mediaType = "image";
        } else if (file.mimetype.startsWith("video/")) {
          video = `/uploads/entertainment/${file.filename}`;
          mediaType = "video";
        }
      });
    }
    ...
    await entertainment.create(data);
    ...
    res.redirect("/admin/entertainment/create");
  }
};
...
exports.getEditNews = async (req, res) => {
  try {
    ...
    if (!news) {
      ...
      return res.redirect("/admin/entertainment");
    }
...
exports.postUpdateNews = async (req, res) => {
  try {
    ...
    await entertainment.findByIdAndUpdate(req.params.id, {
      headline,
      location,
      description,
      highlightText: highlights,
    ...
    res.redirect(`/admin/entertainment/edit/${req.params.id}`);
  }
};
...
exports.downloadVideoWithOverlay = async (req, res) => {
  try {
    ...
    // file paths
    const videoPath = path.join(__dirname, "../../../uploads/entertainment", path.basename(news.video));
