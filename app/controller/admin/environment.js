...
exports.postCreateNews = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        if (file.mimetype.startsWith("image/")) {
          image = `/uploads/environment/${file.filename}`;
          mediaType = "image";
        } else if (file.mimetype.startsWith("video/")) {
          video = `/uploads/environment/${file.filename}`;
          mediaType = "video";
        }
      });
    }
    ...
    res.redirect("/admin/environment/create");
  }
};
...
exports.getEditNews = async (req, res) => {
  try {
    ...
    if (!news) {
      ...
      return res.redirect("/admin/environment");
    }
...
exports.postUpdateNews = async (req, res) => {
  try {
    ...
    await environment.findByIdAndUpdate(req.params.id, {
      headline,
      location,
      description,
      highlightText: highlights,
    ...
    res.redirect(`/admin/environment/edit/${req.params.id}`);
  }
};
...
exports.downloadVideoWithOverlay = async (req, res) => {
  try {
    ...
    // file paths
    const videoPath = path.join(__dirname, "../../../uploads/environment", path.basename(news.video));
