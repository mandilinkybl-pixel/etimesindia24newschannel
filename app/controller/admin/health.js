...
exports.postCreateNews = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        if (file.mimetype.startsWith("image/")) {
          image = `/uploads/health/${file.filename}`;
          mediaType = "image";
        } else if (file.mimetype.startsWith("video/")) {
          video = `/uploads/health/${file.filename}`;
          mediaType = "video";
        }
      });
    }
    ...
    res.redirect("/admin/health/create");
  }
};
...
exports.postUpdateNews = async (req, res) => {
  try {
    ...
    res.redirect(`/admin/health/edit/${req.params.id}`);
  }
};
...
exports.downloadVideoWithOverlay = async (req, res) => {
  try {
    ...
    // file paths
    const videoPath = path.join(__dirname, "../../../uploads/health", path.basename(news.video));
...
ctx.fillText((news.headline || "").substring(0, 60).toUpperCase(), W / 2, topBarH * 0.72);
    ...
    // health clock
    const now = new Date();
    const healthTime = `${now.getHours().toString().padStart(2,"0")}:${now.getMinutes().toString().padStart(2,"0")}:${now.getSeconds().toString().padStart(2,"0")}`;
    ctx.fillStyle = "#b30000";
    ctx.font = `bold ${Math.floor(topBarH * 0.25)}px Arial`;
    ctx.textAlign = "right";
    ctx.fillText(healthTime, W - 8, topBarH - 3);
