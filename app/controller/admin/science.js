    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        if (file.mimetype.startsWith("image/")) {
          image = `/uploads/science/${file.filename}`;
          mediaType = "image";
        } else if (file.mimetype.startsWith("video/")) {
          video = `/uploads/science/${file.filename}`;
          mediaType = "video";
        }
      });
    }
...
exports.getEditNews = async (req, res) => {
  try {
    const news = await science.findById(req.params.id);
    if (!news) {
      req.flash("error", "News not found");
      return res.redirect("/admin/science");
    }
...
exports.postUpdateNews = async (req, res) => {
  try {
    ...
    await science.findByIdAndUpdate(req.params.id, {
      headline,
      location,
      description,
      highlightText: highlights,
    ...
    res.redirect(`/admin/science/edit/${req.params.id}`);
  }
};
...
exports.downloadVideoWithOverlay = async (req, res) => {
  try {
    ...
    // file paths
    const videoPath = path.join(__dirname, "../../../uploads/science", path.basename(news.video));
