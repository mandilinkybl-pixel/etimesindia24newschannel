...
exports.postCreateNews = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        if (file.mimetype.startsWith("image/")) {
          image = `/uploads/world/${file.filename}`;
          mediaType = "image";
        } else if (file.mimetype.startsWith("video/")) {
          video = `/uploads/world/${file.filename}`;
          mediaType = "video";
        }
      });
    }
    ...
    res.redirect("/admin/world/create");
  }
};
...
exports.getEditNews = async (req, res) => {
  try {
    const news = await world.findById(req.params.id);
    if (!news) {
      req.flash("error", "News not found");
      return res.redirect("/admin/world");
    }
...
exports.postUpdateNews = async (req, res) => {
  try {
    ...
    await world.findByIdAndUpdate(req.params.id, {
      headline,
      location,
      description,
      highlightText: highlights,
    ...
    res.redirect(`/admin/world/edit/${req.params.id}`);
  }
};
...
exports.downloadVideoWithOverlay = async (req, res) => {
  try {
    ...
    // file paths
    const videoPath = path.join(__dirname, "../../../uploads/world", path.basename(news.video));
