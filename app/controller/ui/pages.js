...
home = async (req, res) => {
  try {
    ...
    const business = await Business.find().sort({ createdAt: -1 }).limit(6).lean();
    const crime = await Crime.find().sort({ createdAt: -1 }).limit(6).lean();
    const live = await Live.find().sort({ createdAt: -1 }).limit(6).lean();
    const health = await Health.find().sort({ createdAt: -1 }).limit(6).lean();
    const environment = await Environment.find().sort({ createdAt: -1 }).limit(6).lean();
    const education = await Education.find().sort({ createdAt: -1 }).limit(6).lean();
    const entertainment = await Entertainment.find().sort({ createdAt: -1 }).limit(6).lean();
    const national = await National.find().sort({ createdAt: -1 }).limit(6).lean();
    const politics = await Politics.find().sort({ createdAt: -1 }).limit(6).lean();
    const sports = await Sports.find().sort({ createdAt: -1 }).limit(6).lean();
    const technology = await Technology.find().sort({ createdAt: -1 }).limit(6).lean();
    const world = await World.find().sort({ createdAt: -1 }).limit(6).lean();
    const science = await Science.find().sort({ createdAt: -1 }).limit(6).lean();

    res.render("ui/home", {
      title: "E-TIMES-INDIA-24",
      user: req.user || null,
      livevideo, // Pass livevideo directly
      sections: {
        business,
        crime,
        live,
    ...
    login = async (req, res) => {
        try {
            res.render("ui/login", {   // ← this renders views/ui/login.ejs
...
getPlansPage = async (req, res) => {
  try {
    const dbPlans = await subscriptionplan.find({ status: 'active' }).sort({ price: 1 });
    ...
    res.render('ui/plans', { plans,user:req.user || null });
  } catch (err) {
    console.error(err);
    res.status(500).send('Server Error');
  }
...
getNewsDetails = async (req, res) => {
  try {
    ...
    res.render("ui/detail", {
      ...
      user: req.user || null, // For future login features
    });
  } catch (error) {
